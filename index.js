...
┃──────〘  *REGLAS* 〙───────
┃━━━━━━━━━━━━━━━━━━━━
┠⊷️ 1° No saturar al bot
┠⊷️ 2° Pasar pack al priv del bot
┠⊷️ 3° No confies en las morras
┠⊷️ 4° Si usas repetidamente seras bloqueado
┗━━━━━━━━━━━━━━━━━━━━

Porfavor digita [ ${prefix}verify ] para verificar tu cuenta y poder usar al bot.`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Salio/Sacaron a  @${num.split('@')[0]} de todos modos nadie lo queria aqui 👋`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
  ...
  error: {
  stick: 'Intentalo al rato puta :(',
  Iv: 'Error en el link'
  },
  only: {
    group: 'Este comando solo puede ser usado en grupos',
    ownerG: 'Este comando solo puede ser usado por el dueño del grupo',
    ownerB: 'Este comando solo puede ser usado por el creador del bot Mito',
    admin: 'Debes de ser admin para este comando',
    Badmin: 'El bot debe ser admin para este comando',
    daftarB: `Hola, digita [ *${prefix}verify* ] para poder usar el bot como el/ella te uso ati`
  }
}
...
case 'menu':
hasil = `                                ────────────────
Hola *${pushname}* digita [ ${prefix}menu ] para ver mis funciones mamalonas
                                              ────────────────`
reply(hasil)
        break
}
...
〘 *INFO BOT* 〙
‣ Prefijo:  *「 ${prefix} 」*
‣ Yo soy: Monserrat bot
‣ Conección: *Https://Mito.net*
...
//--Info bot
  ...
  ‣ *Dueño* : 𝑴𝑰𝑻𝑶
  ‣ *Prefijo* : ${prefix}
  ‣ *Usuarios bloqueados por putas* : ${blocked.length}
...
pok = await getBuffer(nimek)
Lxa.sendMessage(from, pok, image, {
  quoted: mek, caption: `Monserrat bot`
})
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Monserrat bot'})
						fs.unlinkSync(ran)
					})
					break
...
case 'daftar':
  case 'register':
if (isRegister) return reply('Tu cuenta ya estaba verificada asi que deja de joder')
...
tm = `verificación completa digita [ ${prefix}Menu ] para ver la lista de funciones mamalonas`
reply(tm)
break
...
break
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Que hueva alrato...')
					if (Number(args[0]) === 1) {
  ...
  reply(`        ────────────────\nHola *${pushname}* !!!\n*${prefix}${command}*\nNo esta en mi base de datos digita [ *${prefix}Menu*\n ] para ver los disponibles  ────────────────`)
				}
