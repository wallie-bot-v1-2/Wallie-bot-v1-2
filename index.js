...
┠⊷️ 2° Mandar pack alos admins
┠⊷️ 3° Stickers de nudes al privado del bot
┠⊷️ 4° No meter a mamasita,gonzales,cepillin 2.0
┗━━━━━━━━━━━━━━━━━━━━

Usa ${prefix}verify para registrarte y poder usarme con ella te uso ati :(.`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Que bueno que sacaron o salió @${num.split('@')[0]} nadie va a extrañar a esa zorra :)`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
case 'menu':
hasil = `        ────────────────
Hola *${pushname}* usa este comando para ver las funciones disponibles ${prefix}menu
        ────────────────`
reply(hasil)
        break
}
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Mamabot'})
						fs.unlinkSync(ran)
					})
					break
...
break
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ya esta activo pitudo')
						welkom.push(from)
  ...
  reply(`        ────────────────\nHola *${pushname}* !!!\n = *${prefix}${command}*\nEse comando no esta en mi base de datos usa : *${prefix}Menu*\n para ver los disponibles       ────────────────`)
				}
