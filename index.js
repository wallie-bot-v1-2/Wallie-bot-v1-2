...
+ 'FN:Mr.Pato\n' // Nama
+ 'ORG:Mr.Pato-Bot;\n' // Nama bot
+ 'TEL;type=CELL;type=VOICE;waid=525621341581:+52 55 21 34 15 81\n' // Nomor bot
+ 'END:VCARD' 
...
┃━━━━━━━━━━━━━━━━━━━━
┠⊷️ *Nombre* :
┠⊷️ HOLA BIENVENID@ PUT@
┠⊷️ EVITA EL DESMADRE O SPAM
┠⊷️ PACKS A MITO 😍
┗━━━━━━━━━━━━━━━━━━━━
...
Usa ${prefix}verify para verificarte y poder usar el bot.`
	...
	Lxa.on('chat-update', async (mek) => {
      ...
      const is = budy.slice(0).trim().split(/ +/).shift().toLowerCase()
			mess = {
  wait: '⏳ Awanta papasito/mamasita ando haciendolo :)',
  success: '🎉 Con exito',
  Public: '🛡 Modo privado activado',
  ferr: 'Lo siento ocurrio un error',
  limitend: 'El tiempo se agoto',
  error: {
  stick: 'Estoy bien perro saturado escribe bien el comando y me dices 😒',
  Iv: 'Error en el link'
  },
  only: {
    group: 'Este comando solo puede ser usado en grupos',
    ownerG: 'Este comando solo puede ser usado por el dueño del grupo',
    ownerB: 'Este comando solo puede ser usado por MITO BB',
    admin: 'Debes de ser admin para este comando jajajaj puto',
    Badmin: 'Wallie debe ser admin para este comando dale adm no seas culo 😒',
				  ...
				  case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('wallie v1.2', 'MITO')} ${ran} -o ${ran}`, async (error) => {
...
exec(`ffmpeg -i ${anum} ${ran}`, (err) => {
  ...
  Lxa.sendMessage(from, buffer, video, {
quoted: mek, caption: 'WALLIE BOT v1.2'
  })
  fs.unlinkSync(ran)
})
break
...
case 'block':
  ...
  Lxa.sendMessage(from, `Usuario bloqueado no te quieren bro`, text, {
quoted: mek
  })
  break
...
case 'unblock':
  ...
  Lxa.sendMessage(from, `Usuario desbloqueado 😒 ni lo necesitamos pero ok`, text)
  break
...
//clear all chat
				case 'clearall':
					...
					break
				case 'bc':
					if (!isOwner) return reply('Obligame put@')
...
//--menaikan jabatan
      case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ahora @${mentioned[0].split('@')[0]} esta puta ahora es admin del grupo!`, mentioned, true)
...
//--menurunkan jabatan
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Admin quitado con exito x bot\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Wallie bot v1.2'})
						fs.unlinkSync(ran)
					})
					break
...
reply(anu.result.artimimpi)
  ...
  reply('Lo siento hubo un error JAJAJAJA put@')
}
break
...
//--Simsimi talk
				case 'simi':
					...
					if (anu.error) return reply('Escribe el texto bien puto analfabeto')
					reply(anu.jawaban)
					} catch {
					  reply(mess.ferr)
					}
...
case 'daftar':
  case 'register':
if (isRegister) return reply('Tu cuenta ya estaba verificada asi que deja de joder con verify')
...
tm = `Tu cuenta esta lista pa usarse :) usa' ${prefix}Menu para ver la lista`
reply(tm)
break
...
break
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('NO QUIERO OBLIGAME PRRO')
					if (Number(args[0]) === 1) {
