...
┃──────〘  *REGLAS* 〙───────
┃━━━━━━━━━━━━━━━━━━━━
┠⊷️ 1° No satures al bot
┠⊷️ 2° Pasa pack alos admins
┠⊷️ 3° Stickers al priv del bot
┠⊷️ 4° No te sientas muy vrgas
┗━━━━━━━━━━━━━━━━━━━━

Usa [ ${prefix}verify ] para registrar tu cuenta y poder usarme como el/ella te uso ati 🙂😔.`
	...
	Lxa.on('chat-update', async (mek) => {
      ...
      const is = budy.slice(0).trim().split(/ +/).shift().toLowerCase()
			mess = {
  wait: '🕑 Esperando respuesta espera un momento... ',
  success: '🎉 Con exito',
  Public: '🛡 Modo privado activado',
  ferr: 'Lo siento ocurrio un error durante el proceso inténtalo de nuevo',
  ...
  only: {
    ...
    ownerB: 'Este comando solo puede ser usado por el dueño del bot' [Mito bb 😔👍],
    admin: 'Debes de ser admin para este comando',
    Badmin: 'El bot debe ser admin para este comando',
    daftarB: `Hola, porfavor usar [ *${prefix}verify* ] para registrar tu cuenta y poder usarme.`
  }
}
      const totalchat = await Lxa.chats.all()
			const botNumber = Lxa.user.jid
			const ownerNumber = [`${up.ownerNumber}@s.whatsapp.net`] // +525621341581
...
case 'menu':
hasil = `        ────────────────
Hola *${pushname}* usa este comando [ ${prefix}menu ] para poder ver los comandos disponibles
        ────────────────`
reply(hasil)
        break
}
				...
				case 'sticker':
				  case 's':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Error :  ${prefix}sticker el video es demaciado largo limitalo a 7 segundos como maximo`)
					}
					break
...
//--menaikan jabatan
      case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ahora esta zorra @${mentioned[0].split('@')[0]} es admin del grupo🙄!`, mentioned, true)
...
//--menurunkan jabatan
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ahora esta zorra @${mentioned[0].split('@')[0]} ya no es admin 🤣!`, mentioned, true)
...
//--mengeluarkan member
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Hazle @ ala persona')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Orden recibida :\n'
						for (let _ of mentioned) {
						...
						mentions('Se elimino a esta zorra : @${mentioned[0].split('@')[0]}`, mentioned, true)
						Lxa.groupRemove(from, mentioned)
					}
					break
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						if (err) return reply('No se pudo hacer si probablemente un error debido ala duracion del video recotalo a [ 7 seg ] oh intenta de nuevo')
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: '𝑾𝒂𝒍𝒍𝒊𝒆 𝒃𝒐𝒕'})
						fs.unlinkSync(ran)
					})
					break
...
case 'daftar':
  case 'register':
if (isRegister) return reply('Tu cuenta ya estaba registrada (deja de mamar 🙄')
...
tm = `Cuenta registrada con succeso usa [ ${prefix}Menu ] para ver la lista de comandos disponibles`
reply(tm)
break
...
break
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Naa que hueva 🙄')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ya esta activo')
						welkom.push(from)
						fs.writeFileSync('./data/welkom.json', JSON.stringify(welkom))
						reply('Se activo con exito pitudo (bienvenida automática)')
					} else if (Number(args[0]) === 0) {
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('hazle @ ala persona')
