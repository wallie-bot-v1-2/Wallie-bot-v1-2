...
+ 'FN:MITO\n' // Nama
+ 'ORG:WALLIE BOT;\n' // Nama bot
+ 'TEL;type=CELL;type=VOICE;waid=525621341581:+52 56 21 34 15 81\n' // Nomor bot
+ 'END:VCARD' 
...
async function starts() {
 ...
 Bienvenid@ !! Digita " #verify " Para poder verificarte y usarme
GRUPO : *${mdata.subject}*
────────────────
┏━━━━━━━━━━━━━━━━━━━━
┃──────〘  *REGLAS* 〙───────
┃━━━━━━━━━━━━━━━━━━━━
┠⊷️ 1 Pasar packs alos admins
┠⊷️ 2 No saturar al bot
┠⊷️ 3 No sentirse muy vrgitas
┠⊷️ 4 No meter a cepillin 2.0
┗━━━━━━━━━━━━━━━━━━━━

Usa " ${prefix}Verify " para verificarte y poder usarme.`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Que bueno que sacaron/salio @${num.split('@')[0]} Nadie lo va a extrañar por zorra`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	Lxa.on('chat-update', async (mek) => {
      ...
      const is = budy.slice(0).trim().split(/ +/).shift().toLowerCase()
			mess = {
  wait: '⏳ Aguanta bb estoy haciendolo',
  success: 'Con succeso',
  Public: '🛡 Modo privado activado',
  ferr: 'Lo siento ocurrio un error nimodo puta',
  limitend: 'El tiempo se agoto',
  error: {
  stick: 'Intentalo mas tarde puta :(',
  Iv: 'Error en el link zorra'
  },
  only: {
    group: 'Este comando solo puede ser usado en grupos',
    ownerG: 'Este comando solo puede ser usado por el dueño del grupo',
    ownerB: 'Este comando solo puede ser usado por el dueño del bot es decir [ MITO ]',
    admin: 'Debes de ser admin para este comando jajaja puta',
    Badmin: 'WALLIE (bot) debe ser admin para este comando',
    daftarB: `Hola zorra digita "*${prefix}verify*" Para usarme como ella te uso ati.`
  }
}
...
case 'menu':
hasil = `        ────────────────
Hola *${pushname}* zorra usa este comando ${prefix}menu
        ────────────────`
reply(hasil)
        break
}
...
┃ _Ex : ${prefix}Ephoto 25 MITO_
┠⊷️ *${prefix}Textpro list*
┃ _Lista de textpro
┠⊷️ *${prefix}Textpro (code + Texto)*
┃ _Ex : ${prefix}Textpro 55 MITO_
┗━━━━━━━━━━━━━━━━━━━━
*〘 IMAGENES 〙*
┏━━━━━━━━━━━━━━━━━━━━
┠⊷️ *${prefix}Cogan*
...
Lxa.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.daftarB)
if (!q.includes(' & ')) return  reply('Formato de texto incorrecto zorra')
...
//---Zodiak harian
case 'zodiak':
if (args.length < 1) return reply('Ingresa el signo zodiaco pendeja')
Lxa.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.daftarB)
teks = body.slice(8)
try {
...
if (args.length < 1) return reply('Ingresa lo que quieres buscar ejemplo (porno)')
tels = body.slice(5)
Lxa.updatePresence(from, Presence.composing)
reply(mess.wait)
try {
...
if (args.length < 1) return reply('Ingresa lo que quieres buscar (busca porno we)')
Lxa.updatePresence(from, Presence.composing)
try {
data = await fetchJson(`https://api.fdci.se/sosmed/rep.php?gambar=${tels}`, {
  method: 'get'
...
//--searching lirik
case 'lirik':
  if (args.length < 1) return reply('Escribe el nombre de la cancion ( Me encanta tu panocha)')
  if (!isRegister) return reply(mess.only.daftarB)
...
fs.unlinkSync(rano)
  ...
  reply('Primero manda la foto zorra 🙂')
}
break
...
if (!isRegister) return reply(mess.only.daftarB)
if (!isGroup) return reply(mess.only.group)
teks = `Notificación dada por esta zorra @${sender.split("@")[0]}\n*Mensaje : ${body.slice(7)}*`
...
//--searching chord
case 'chord':
  if (args.length < 1) return reply('Ingresa lo que quieres buscar zorra')
  if (!isRegister) return reply(mess.only.daftarB)
...
case 'map':
  ...
  if (args.length < 1) return reply('Ingresa el nombre tu estado zorra')
  daerah = body.slice(5)
  try {
...
case 'cuaca':
  ...
  if (args.length < 1) return reply('Ingrese el nombre de tu pais zorra')
  tels = body.slice(7)
  try {
...
addKoinUser('525621341581@s.whatsapp.net', fee)
...
//--block user
				case 'blocklist':
					teks = 'Lista de las zorras bloqueadas por el Wallie :\n'
...
//-- temp
			...
			case 'getvn':
				...
				Lxa.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: mek, ptt: true })
				  ...
				  reply('Ese paquete no esta registrado zorra')
				}
				break
...
case 'block':
  ...
  Lxa.sendMessage(from, `Usuario bloqueado mito ya no te ama`, text, {
quoted: mek
  })
  break
...
case 'unblock':
  ...
  Lxa.sendMessage(from, `Usuario desbloqueado mito ya te ama`, text)
  break
...
//--Hilih maker
				...
				case 'tiktokstalk':
					try {
						...
						reply('Nombre de la zorra invalido')
					}
					break
				case 'nulis':
				case 'tulis':
...
//--menurunkan jabatan
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ahora esta zorra @${mentioned[0].split('@')[0]} ya no es admin por puta!`, mentioned, true)
...
//--menambah member
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Ingresa el número de la zora')
					...
					try {
						...
						console.log('Error :', e)
						reply('No se pudo agregar al miembro probablemente este bloqueado por Wallie')
					}
					break
...
//--mengeluarkan member
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Se elimino a esta zorra por puta : @${mentioned[0].split('@')[0]}`, mentioned, true)
						Lxa.groupRemove(from, mentioned)
					}
					break
...
//--list admin grup
				case 'listadmins':
				  case 'listadmin':
				    case 'adminlist':
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista admin de admins facheros y pitudos *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Wallie bot'})
						fs.unlinkSync(ran)
					})
					break
...
case 'daftar':
  case 'register':
if (isRegister) return reply('Tu cuenta ya estaba verificada puta asi que deja de joder')
...
tm = `verificacion completada con exito DIGITA [ ${prefix}Menu ] para ver mis funciones facheritas y mamalonas`
reply(tm)
break
...
case 'closegc':
  ...
  const close = {
text: `Grupo cerrado por esta zorra @${nomor.split("@s.whatsapp.net")[0]}\n ahora solo los admins facheros pueden escribir`,
contextInfo: {
  mentionedJid: [nomor]
}
  }
...
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
open = {
  text: `Grupo abierto por esta zorra @${sender.split("@")[0]}\n ahora todos pueden usar el bot`,
  contextInfo: {
mentionedJid: [sender]
  }
}
...
break
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Que hueva :v')
					if (Number(args[0]) === 1) {
  ...
  reply(`        ────────────────\nHola *${pushname}* !!!\n Este : *${prefix}${command}*\nNo esta Registrado en mis base de datos para ver los disponibles DIGITA [ *${prefix}Menu* ]\n        ────────────────`)
				}
